...
class Simulator(ABC):
    ...
    def _pos_from_state(self, state: npt.NDarray) -> npt.NDarray:
        return state[: self.dim]

    def _vel_from_state(self, state: npt.NDarray) -> npt.NDarray:
        return state[self.dim :]

    def atmosphere(self, state: npt.NDarray, time: float) -> float:
    ...
    def atmosphere_velocity(
        self, state: npt.NDarray, time: float
    ) -> npt.NDarray:
    ...
    def _drag_accel(self, state: npt.NDarray, time: float) -> npt.NDarray:
        ...
        """
        relative_velocity = self._vel_from_state(
            state
        ) - self.atmosphere_velocity(state, time)
        accel = (
            -(1 / (2 * SATELLITE_MASS))
